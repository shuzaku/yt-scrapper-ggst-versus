...
var channel = {
      id: ObjectId('63eeff19a05379e7ca661eb0'),
      youtubeId:'UCG3MlAZvF04whMkoFW7T5tg',
      name: 'GGST VERSUS',
      channelVideos: []
    }
...
function getVideos(channel) {
  ...
  https.get(route, (resp) => {
    ...
    resp.on('end', () => {
      ...
      //sometimes this channel does player vs montages
...
function getPlayer1CharacterId(title){
  ...
  var characterId = getCharacterIdByName(characterName.trim());
  return characterId;
}
...
function getPlayer2CharacterId(title){
  if(title.includes('(')){
    ...
    var characterName = title.slice(1,end);
    var characterId = getCharacterIdByName(characterName.trim());
    return characterId;
  }
  else {
